-- FIXED Finance Database Schema for Supabase
-- Resolves "generation expression is not immutable" error by using triggers instead of generated columns

-- 1. Create main transactions table (without generated column)
CREATE TABLE IF NOT EXISTS transactions (
    ...
    -- Monthly partition key (populated by trigger)
    month_year TEXT
);

-- Create trigger function to populate month_year
CREATE OR REPLACE FUNCTION update_month_year()
RETURNS TRIGGER AS $$
BEGIN
...
-- Create trigger to automatically populate month_year on insert and update
CREATE TRIGGER trigger_update_month_year
    BEFORE INSERT OR UPDATE ON transactions
    FOR EACH ROW
    EXECUTE FUNCTION update_month_year();
